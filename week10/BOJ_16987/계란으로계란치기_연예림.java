...
public class 계란으로계란치기_연예림 {
  ...
  public static void getEggMax(int idx, int cnt) {
    ...
    boolean target = false; // 깰 계란이 있는지 확인
    if (eggs[idx][0] > 0) {   // 손에 든 달걀이 깨져있는지
      for (int i = 0; i < N; i++) { // 오른쪽 달걀부터 치기x => 모든 달걀 치기

        if (i == idx || eggs[i][0] <= 0) continue; // 이미 깨진 달걀, 내 달걀을 치지 않음
